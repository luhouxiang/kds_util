...
def init_logger(path="../log", name="server", level="INFO"):
    ...
    _InSideLog.user_file_handler = TimedRotatingFileHandler(
        os.path.join(_InSideLog.main_log_dir, '%s.log' % name), date_format='%Y%m%d', bubble=True, level=level)
    _InSideLog.user_file_handler.formatter = user_handler_log_formatter

    _InSideLog.system_file_handler = TimedRotatingFileHandler(
        os.path.join(_InSideLog.main_log_dir, '%s.log' % 'system'), date_format='%Y%m%d', bubble=True, level=level)
